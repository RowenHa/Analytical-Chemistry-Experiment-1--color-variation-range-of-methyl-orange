...
def extract_hsv_features(
	...
	"""提取图像的平均 HSV 特征并返回基础统计信息。"""
	_, mean_hsv, center_xy, picked_count = collect_similar_pixels(
		image_path=image_path,
		target_count=target_count,
		threshold=threshold,
	)
	return float(mean_hsv[0]), float(mean_hsv[1]), float(mean_hsv[2]), center_xy, picked_count
...
def main() -> None:
	...
	parser.add_argument("--json", dest="as_json", action="store_true", help="以 JSON 输出 HSV 结果")
			...
			json.dumps(
				{
					...
					"center_x": int(center_xy[0]),
					"center_y": int(center_xy[1]),
				},
				ensure_ascii=False,
			)
		)
